...
namespace CalendarProxy.Helpers
{
    ...
    public class EventManager
    {
        ...
        /// </summary>
        private VCalendarParser calendar = null;
        ...
        {
            List<VEvent> eventsToRemove = calendar.VCalendar.Events.Where(evnt => evnt.Summary.Value.Contains(stringPart)).ToList();
            foreach (var evnt in eventsToRemove)
                calendar.VCalendar.Events.Remove(evnt);
        }

        /// <summary>
        /// Removes all events that are shorter than given number of minutes. Events with duration time equal to a given parameter will not be removed.
        /// </summary>
        /// <param name="minutes"></param>
        private void HideEventsShorterThanMinutes(int minutes)
        {
            List<VEvent> eventsToRemove = calendar.VCalendar.Events.Where(evnt => evnt.DurationBasedOnDates().TotalMinutes < minutes).ToList();
            foreach (var evnt in eventsToRemove)
                calendar.VCalendar.Events.Remove(evnt);
        }
        ...
        {
            List<VEvent> eventsToRemove = calendar.VCalendar.Events.Where(evnt => projectsToSkip.Contains(evnt.ProjectName())).ToList();
            foreach (var evnt in eventsToRemove)
                calendar.VCalendar.Events.Remove(evnt);
        }
